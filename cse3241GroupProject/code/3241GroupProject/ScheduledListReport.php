...
<?php 
    ...
    // get and draw table of scheduled patients
    $query="
        select name, patientid, dates, manufacturer from ((
        select name, a.patientid, dates, batchid from(
        select patientid, dates, batchid from waitlist join dose on status = 'assigned' and dose.trackingnumber = waitlist.trackingnumber)
    ...
    echo "<h1>List of Scheduled Patients</h1>
